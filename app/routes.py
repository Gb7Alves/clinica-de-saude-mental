...
def registrar():
    if request.method == 'POST':
        ...
        if usuario_existente:
            ...
            db.session.commit()
            flash('Cadastro realizado com sucesso! Sua conta está inativa. Aguarde a liberação do administrador.', 'success')
...
def index():
    ...
    # CORREÇÃO: Contador de Agendamentos (Total de agendamentos cadastrados)
...
def novo_paciente():
    ...
    if request.method == 'POST':
        ...
        try:
            ...
            # CORREÇÃO 1: Verificar se CPF já existe antes de salvar
            paciente_existente = Paciente.query.filter_by(cpf=cpf).first()
            if paciente_existente:
                flash('Já existe um paciente cadastrado com este CPF. Verifique os dados ou utilize outro CPF.', 'danger')
...
def editar_paciente(id):
    ...
    if request.method == 'POST':
        ...
        try:
            ...
            # CORREÇÃO 1: Verificar se CPF já existe em outro paciente
            paciente_existente = Paciente.query.filter(
                Paciente.cpf == cpf,
                Paciente.id != id
            ).first()
            if paciente_existente:
                flash('Já existe outro paciente cadastrado com este CPF. Verifique os dados ou utilize outro CPF.', 'danger')
...
def novo_profissional():
    ...
    if request.method == 'POST':
        ...
        try:
            ...
            # CORREÇÃO 3: Verificar se CPF já existe para profissionais
            profissional_existente = Profissional.query.filter_by(cpf=cpf).first()
            if profissional_existente:
                flash('Já existe um profissional cadastrado com este CPF. Verifique os dados ou utilize outro CPF.', 'danger')
...
def editar_profissional(id):
    ...
    if request.method == 'POST':
        ...
        try:
            ...
            # CORREÇÃO 3: Verificar se CPF já existe em outro profissional
            profissional_existente = Profissional.query.filter(
                Profissional.cpf == cpf,
                Profissional.id != id
            ).first()
            if profissional_existente:
                flash('Já existe outro profissional cadastrado com este CPF. Verifique os dados ou utilize outro CPF.', 'danger')
...
def nova_consulta():
    ...
    if request.method == 'POST':
        ...
        try:
            ...
            # Atualizar status do agendamento para 'Realizado' se agendamento_id foi fornecido
            agendamento_id = request.form.get('agendamento_id')
            if agendamento_id:
                try:
                    agendamento = Agendamento.query.get(int(agendamento_id))
                    if agendamento:
                        agendamento.status = 'Realizado'
                        db.session.commit()
                except:
                    pass  # Se houver erro ao atualizar agendamento, continua mesmo assim
    ...
    agendamento_id_pre = request.args.get('agendamento_id')
    ...
    return render_template(
        ...
        agendamento_id_pre=agendamento_id_pre,
...
def novo_agendamento():
    ...
    if request.method == 'POST':
        ...
        try:
            ...
            # Se "Outros" foi selecionado, usar o campo customizado
            if consultorio_sala == 'Outros':
                consultorio_sala = request.form.get('consultorio_sala_customizada')
            ...
            # CORREÇÃO 2: Bloqueio de Conflitos de Agendamento (Regra Completa)
            paciente_id = request.form['paciente_id']
            
            # 1. Mesmo paciente + mesmo dia + mesmo horário (independente da sala)
            conflito_paciente = Agendamento.query.filter(
            ...
            for agend in conflito_paciente:
                if agend.hora_inicio and agend.hora_fim:
                    if not (hora_fim <= agend.hora_inicio or hora_inicio >= agend.hora_fim):
                        flash('O paciente já possui um agendamento para este dia e horário.', 'danger')
                        pacientes = Paciente.query.all()
                        profissionais = Profissional.query.all()
                        consultorios = Consultorio.query.all()
                        return render_template(
            ...
            # 2. Mesmo dia + mesmo horário + mesma sala (independente do paciente)
            if consultorio_sala:
                conflito_sala = Agendamento.query.filter(
                    Agendamento.consultorio_sala == consultorio_sala,
                    Agendamento.data_agendamento.cast(db.Date) == data_agendamento.date(),
                    Agendamento.status.in_(['Agendado', 'Realizado'])
                ).all()
                
                for agend in conflito_sala:
                    if agend.hora_inicio and agend.hora_fim:
                        if not (hora_fim <= agend.hora_inicio or hora_inicio >= agend.hora_fim):
                            flash('Este horário já está ocupado para a sala selecionada. Escolha outro horário ou sala para continuar.', 'danger')
                            pacientes = Paciente.query.all()
                            profissionais = Profissional.query.all()
                            consultorios = Consultorio.query.all()
                            return render_template(
...
def editar_agendamento(id):
    ...
    if request.method == 'POST':
        ...
        try:
            ...
            # CORREÇÃO 2: Bloqueio de Conflitos de Agendamento (Regra Completa - Edição)
            paciente_id = request.form['paciente_id']
...
def editar_acesso(id):
    ...
    if request.method == 'POST':
        ...
        try:
            ...
            # Processar alteracao de senha se fornecida
            nova_senha = request.form.get('nova_senha')
            confirmar_senha = request.form.get('confirmar_senha')
            
            if nova_senha or confirmar_senha:
                # Se um dos campos foi preenchido, ambos sao obrigatorios
